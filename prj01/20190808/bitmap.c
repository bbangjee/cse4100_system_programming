...
struct bitmap *bitmap_expand (struct bitmap *bitmap, int size) {
  if (bitmap == NULL || size < 0) return NULL;

  size_t before_size = bitmap_size(bitmap); 
  size_t new_size = before_size + size;

  elem_type *new_bits = realloc(bitmap->bits, byte_cnt(new_size));
  if (new_bits == NULL) return NULL;
  
  bitmap->bits = new_bits;
  bitmap->bit_cnt = new_size;

  bitmap_set_multiple(bitmap, before_size, size, false);
