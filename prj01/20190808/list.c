...
/* Swap two list elements in paramters. return value: none */
...
/* Shuffle elements of LIST in the parameter */
void list_shuffle(struct list *list)
{
  ...
  struct list_elem **elements = malloc(sizeof(struct list_elem *) * size);
  ...
  for (size_t j = 0; j < size - 1; j++) {
    if (size - j - 1 <= 0)
      break;  
    size_t k = j + 1 + rand() % (size - j - 1);
    
    list_swap(elements[j], elements[k]);
    struct list_elem *temp = elements[j];
